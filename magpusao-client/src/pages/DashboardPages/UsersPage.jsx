...
export default function UsersPage() {
  ...
  const handleSave = async () => {
    ...
    try {
      if (editingUser) {
        const updatedUser = await updateUser(editingUser._id || editingUser.id, cleanedUser);
        setUsers((currentUsers) => currentUsers.map((user) => (user.id === editingUser.id ? { ...updatedUser, id: editingUser.id } : user)));
      } else {
        const createdUser = await createUser(cleanedUser);
